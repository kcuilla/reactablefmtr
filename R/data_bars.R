...
#### need to test to see if everything passes with missing values
...
qbs %>%
  select(name, team_logo_espn, team_color, n_plays, cpoe, epa) %>%
  # mutate(n_plays = case_when(
  #   name == "M.Trubisky" ~ 0,
  #   TRUE ~ n_plays
  # )) %>%
  ...
  mutate(icon_colors = case_when(
    icons == "check" ~ "green",
    icons == "times" ~ "red",
    TRUE ~ "grey"
  )) %>%
  mutate(fill_color_refs = case_when(
    epa > 0 ~ "green",
    TRUE ~ "grey"
  )) %>%
reactable(.,
          pagination = FALSE,
          columns = list(team_logo_espn = colDef(show = FALSE),
                         team_color = colDef(show = FALSE),
                         icons = colDef(show = FALSE),
                         icon_colors = colDef(show = FALSE),
                         n_plays = colDef(
            cell = data_bars(.,
                             fill_color_ref = "fill_color_refs",
                             bold_text = TRUE,
...
data %>%
  mutate(images = case_when(
    Make == "Geo Metro" ~ "https://upload.wikimedia.org/wikipedia/commons/9/9f/Iris_virginica.jpg",
    TRUE ~ "https://upload.wikimedia.org/wikipedia/commons/7/7a/Iris_versicolor.jpg"
  )) %>%
reactable(.,
          defaultColDef = colDef(
            cell = data_bars(., fill_color = viridis(4),
...
data %>%
  ...
  mutate(icons = case_when(
    str_detect(Group, "Red") ~ "circle",
    str_detect(Group, "Blue") ~ "square",
    str_detect(Group, "Green") ~ "check",
    TRUE ~ "circle"
  )) %>%
  mutate(icon_colors = case_when(
    str_detect(Group, "Red") ~ "green",
    str_detect(Group, "Blue") ~ "orange",
    str_detect(Group, "Green") ~ "red",
    TRUE ~ "circle"
  )) %>%
  reactable(.,
            defaultColDef = colDef(
...
reactable(data,
          columns = list(MPG.city = colDef(
            cell = data_bars(data, fill_color = c("green","red"), text_position = "inside-end", fill_gradient = TRUE, bold_text = TRUE))))
