...
As you can see above, to apply a theme to a table, you just need to simply reference the theme name and included closed brackets at the end of the name. The reason why the closed brackets are necessary is because there are additional optional arguments that you can apply to the theme, such as the font family, size, and color. Below are the full options of optional arguments you can change to the theme:
...
data %>%
  reactable(.,
    theme = slate(font_family = "Courier New", font_size = 18, header_font_size = 20, header_font_color = "darkorange", cell_padding = 8),
    defaultColDef =
      colDef(
        cell = data_bars(., fill_color = viridis::mako(5), text_position = "inside-end")
  )
...
Additional boostrap-inspired themes include:
