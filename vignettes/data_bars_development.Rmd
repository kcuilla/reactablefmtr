...
### This version of data_bars() is currently available in the development version only. To download the development version from GitHub, run `remotes::install_github("kcuilla/reactablefmtr")`

For example of `data_bars()` used within the version currently available on CRAN, please check the [homepage](https://kcuilla.github.io/reactablefmtr/index.html).

The development version of `data_bars()` has a ton of new features and flexibility that we'll go over using the example dataset created below:
...
reactable(data,
          ...
          columns = list(
            ...
            cell = data_bars(data,
                             ...
                             fill_color_ref = "Primary",
                             text_position = "inside-end")),
            Company = colDef(show = FALSE), ### hide the company column
            Primary = colDef(show = FALSE))) ### hide the color hex column
```
