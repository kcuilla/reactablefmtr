...
By default, the values are now positioned on the "inside-end" of the filled bars:
...
However, you can change the location of the values with `text_position`. In addition to "inside-end", there is also "outside-base", "outside-end", "inside-base", "center", or "none". Below is an example of "outside-base" which is the way the old version of `data_bars()` displayed the values.
...
Values positioned outside the end of the filled bars with "outside-end":
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, fill_color = "lightblue", text_position = "outside-end", number_fmt = scales::percent)
  )
)
```
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, fill_color = viridis(5), number_fmt = scales::percent)
  )
)
```
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, fill_color = viridis(5), brighten_text_color = "gold", number_fmt = scales::percent)
  )
)
```
...
reactable(
  ...
  columns = list(
    ...
    Pct2 = colDef(
      cell = data_bars(data, fill_color = "lightblue", align_bars = "left",  number_fmt = scales::percent)
      )
  )
)
```
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, fill_color = viridis(5), background = "lightgrey", max_value = 1, number_fmt = scales::percent)
  )
)
```
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, bar_height = 40, number_fmt = scales::percent)
  )
)
```
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, bar_height = 3, number_fmt = scales::percent, text_position = "outside-end")
  )
)
```
...
reactable(.,
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(., fill_color_ref = "color_pal", background = "lightgrey", max_value = 1, number_fmt = scales::percent)
  ),
...
reactable(
  ...
  pagination = FALSE,
  defaultColDef = colDef(
    cell = data_bars(data, fill_color = c("#1efffd", "#1e20ff"), fill_gradient = TRUE, background = "lightgrey", text_color = "white", max_value = 1, number_fmt = scales::percent)
  )
)
```
...
The labels can also be positioned in the same fashion that is used for the positive-valued data bars in the examples above:
...
reactable(.,
  ...
  columns = list(
    Change = colDef(
      cell = data_bars(., fill_color = c("lightblue","orange"), number_fmt = scales::percent, text_position = "outside-end"))
  )
)
```
